...
namespace Microsoft.Build.Tasks
{
    ...
    public class FxCompile : ToolTask
    {
        ...
        protected override string GenerateFullPathToTool()
        {
            ...
            {
				string path = Path.Combine(kitRoot, "bin");
				DirectoryInfo directoryInfo = new DirectoryInfo(path);
				path = directoryInfo.GetFiles(ToolName, SearchOption.AllDirectories).Where(file => file.FullName.Replace(path, string.Empty).Contains("x86")).Select(file => file.FullName).FirstOrDefault();
